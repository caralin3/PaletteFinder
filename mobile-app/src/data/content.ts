export const header = 'Eyeshadow Palette Finder'
export const welcomeMsg = 'Discover a palette that best fits your overall needs and preferences by answering a few questions.';
export const welcomeTitle = 'Let\'s Get Swatching!';
export const welcomeButton = 'Let\'s Get Swatching!';
export const welcomeChoice1 = 'View Last Result';
export const welcomeChoice2 = 'Find Palettes';
export const nextButton = 'Next';
export const continueButton = 'Continue';
export const continueMsg = 'Now let’s pick your Eyeshadow Preferences';
export const resultsMsg = 'The results are in\, meet your new favorites!';
export const resultsSaveButton = 'Save Results';
export const resultsResetButton = 'Start Over';
export const homeMsg = 'Most Recent Results';
export const homeEmptyMsg = 'No Recent Results';
export const homeHistoryButton = 'View History';
export const homeResetButton = 'Find Palettes';
export const historyEmptyMsg = 'History is Empty';
export const historyClearButton = 'Clear History';
export const historyResetButton = 'Find Palettes';
export const historyHomeButton = 'Back Home';
